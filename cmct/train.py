...
def main():
    ...
    if not config.branch_lora.enabled:
        # `resolve()` has already forced branch_mlp.cross_weight to 0.0 when
        # there is no LoRA teacher; this only reports that consequence.
    ...
    # EMA teacher alone would leave the head untracked. Hard-copied every step
    # for ema.hard_copy_iters under the "hard_copy" schedule, then a real EMA
    # after that.
    ...
    # initial_lr=1.0 is baked into the param groups via get_parameters(); the
    # LR value itself lives entirely in the LambdaLR below.
    ...
    for macro in range(config.train.iters):
        ...
        if lora_enabled:
            ...
            if in_warmup_lora:
                ...
                loss_u_lora_cross = masked_cross_entropy(logits_u_lora, prob_cross_for_lora, confi)
                loss_lora = (
        ...
        if (
            ...
            or (macro + 1) == config.branch_lora.warmup.iters
            ...
            tags = []
            if (macro + 1) == config.branch_lora.warmup.iters:
            ...
            print(f"[eval] macro {macro + 1}{tag}: teacher_lora {lora_report} | "
                  f"teacher_mlp {acc_mlp:.2f}% | ensemble {ens_report}")
    ...
    print(f"Done. best teacher_lora={best_acc_lora:.2f}% teacher_mlp={best_acc_mlp:.2f}% "
          f"ensemble={best_acc_ens:.2f}%")
