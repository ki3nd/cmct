"""CLIP backbone for `branch_mlp`, with its hardcoded per-dataset prompt lists.

See NOTICE for this module's license terms.

The prompt lists below are literal text, NOT derived from the dataset's class
names: the trained cosine head depends on their exact text and order, so they
are kept as hardcoded strings rather than generated from `classnames`.
...
class ClipBackbone(nn.Module):
    ...
    def set_text_features(self, text_features):
        ...
        branch's LoRA. A class-count or width mismatch is rejected here because
        it would otherwise surface as an opaque shape error inside CMKD.forward.
        """
        expected = self.text_features.shape
        if text_features.shape != expected:
            raise ValueError(
                f"set_text_features expected {tuple(expected)}, got "
