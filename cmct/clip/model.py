...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.transformer = Transformer(
            ...
            attn_mask=self.build_attention_mask()       # attn_mask looks like:
                                                        #   [[ 0, -inf, -inf, -inf],
                                                        #    [ 0,   0, -inf, -inf],
                                                        #    [ 0,   0,   0, -inf],
                                                        #    [ 0,   0,   0,   0]]
        ...
        self.text_projection = nn.Parameter(torch.empty(transformer_width, embed_dim))      # [512, 512] projection layer
    ...
    def build_attention_mask(self):
        ...
        mask.triu_(1)  # zero out the lower diagonal (including the main diagonal)
    ...
    def encode_text(self, text):
        ...
        # torch.arange(x.shape[0]) : the class indices, e.g. [0, 1, ..., n_cls - 1], shape [n_cls]
        # text.argmax(dim=-1)      : index of the max value along the last dim, shape [n_cls]
        # x[torch.arange(x.shape[0]), text.argmax(dim=-1)]: for each sample, the feature at its
        #   max-index token -- [n_cls, 77, 512] --> [n_cls, 1, 512] --> [n_cls, 512]
