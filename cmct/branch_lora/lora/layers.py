...
class LoRALayer():
    ...
    def init_lora_param(self):
        """
        Initialize lora_A / lora_B with a SoRA-style SVD and write the
        residual back into the original weight. This fully replaces the
        default kaiming_uniform + zeros initialisation.
        """
        if self.r <= 0:
            return
        ...
        for param_name, lora_name in self.params_with_lora.items():
            ...
            dtype = W.dtype
            device = W.device
            # 1. Upcast to float32 for the SVD
            weight_f32 = W.data.to(torch.float32)

            U, S, Vh = torch.linalg.svd(weight_f32, full_matrices=False)

            # 2. Pick the singular values
            if self.first_eigen:  # principal components
                Ur, Sr, Vhr = U[:, :self.r], S[:self.r], Vh[:self.r, :]
            else:  # minor components (SoRA's own default)
                Ur, Sr, Vhr = U[:, -self.r:], S[-self.r:], Vh[-self.r:, :]

            Sr = Sr / self.scaling  # divide out scaling here; the forward pass multiplies it back

            # 3. Build lora_A / lora_B
            sqrt_S = torch.diag(torch.sqrt(Sr))
            lora_A = sqrt_S @ Vhr  # (r, d_in)
            lora_B = Ur @ sqrt_S  # (d_out, r)

            # 4. Write back into the trainable parameters
            getattr(self, f'{lora_name}_lora_A').data = lora_A.to(dtype)
            getattr(self, f'{lora_name}_lora_B').data = lora_B.to(dtype)

            # 5. Write the residual back into the original weight and freeze it
