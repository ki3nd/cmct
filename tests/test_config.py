...
def test_imagenet_config_parses_with_reg_lambdas_at_zero():
    ...
    assert cfg.branch_mlp.lambdas.task != 0.0
    assert cfg.branch_mlp.lambdas.source_ce == 0.0
    assert cfg.branch_mlp.lambdas.target_gini == 0.0
...
def test_cmkd_settings_are_required_under_the_imagenet_source(tmp_path):
    p = tmp_path / "bad.yaml"
    p.write_text(read_text(IMAGENET_CONFIG_PATH).replace("  lamb_gamma: 1.0\n", ""))
    with pytest.raises(ValueError, match="lamb_gamma"):
        Config.from_yaml(str(p))


def test_the_reg_lambdas_must_be_zero_under_the_imagenet_source(tmp_path):
    # There is no cosine head to compute the two reg terms against, so a
    # nonzero weight on them would be silently ignored.
    p = tmp_path / "bad.yaml"
    p.write_text(read_text(IMAGENET_CONFIG_PATH).replace(
        "source_ce: 0.0", "source_ce: 0.1"))
    with pytest.raises(ValueError, match="source_ce"):
        Config.from_yaml(str(p))
